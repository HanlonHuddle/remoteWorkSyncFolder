...
class TicTacToe:
    ...
    def __init__(self, n: int):
        ...
        """
        self.rs = [[0, 0] for i in range(n)]
        self.cs = [[0, 0] for i in range(n)]
        self.cs1, self.cs2 = [0, 0], [0, 0]
        self.n = n
    ...
    def move(self, row: int, col: int, player: int) -> int:
        ...
        @return The current winning condition, can be either:
                ...
                2: Player 2 wins.
        """
        rs, cs, cs1, cs2, n = self.rs, self.cs, self.cs1, self.cs2, self.n
        player -= 1
        rs[row][player] += 1
        cs[col][player] += 1
        if row == col:
        ...
        if row + col == n-1:
            cs2[player] += 1
        if (rs[row][player] == n) or (cs[col][player] == n) or (cs1[player] == n) or (cs2[player] == n):
            return player + 1
