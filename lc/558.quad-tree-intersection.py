...
class Solution:
    def intersect(self, quadTree1, quadTree2):
        ...
        if (quadTree1.isLeaf and not quadTree1.val) or (quadTree2.isLeaf and not quadTree2.val):
            return copy.deepcopy(quadTree2) if quadTree1.isLeaf else copy.deepcopy(quadTree1)
        
        nTL = self.intersect(quadTree1.topLeft, quadTree2.topLeft)
        nTR = self.intersect(quadTree1.topRight, quadTree2.topRight)
        nBL = self.intersect(quadTree1.bottomLeft, quadTree2.bottomLeft)
        nBR = self.intersect(quadTree1.bottomRight, quadTree2.bottomRight)

        if nTL.isLeaf and nTR.isLeaf and nBL.isLeaf and nBR.isLeaf and nTL.val == nTR.val == nBL.val == nBR.val != None:
            return Node(nTL.val, True, None, None, None, None)
        else:
            return Node(None, False, nTL, nTR, nBL, nBR)
